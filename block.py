...
class Block(object):
    def __init__(self, shape, x, y, screen, color, rotate_en):
        self.shape = []
        for sh in shape:
            bx = sh[0] * config.BWIDTH + x
            by = sh[1] * config.BHEIGHT + y
            block = pygame.Rect(bx, by, config.BWIDTH, config.BHEIGHT)
    ...
    def draw(self):
        for bl in self.shape:
            ...
            pygame.draw.rect(self.screen, config.BLACK, bl, config.MESH_WIDTH)
    ...
    def remove_blocks(self, y):
        ...
        for shape_i in range(len(self.shape)):
            ...
            if tmp_shape.y < y:
                ...
                tmp_shape.move_ip(0, config.BHEIGHT)
            elif tmp_shape.y > y:
                new_shape.append(tmp_shape)
    ...
    def _update(self):
        for bl in self.shape:
            origX = (bl.x - self.x) / config.BWIDTH
            origY = (bl.y - self.y) / config.BHEIGHT
            rx, ry = self.get_rotated(origX, origY)
            newX = rx * config.BWIDTH + self.x + self.diffx
            newY = ry * config.BHEIGHT + self.y + self.diffy
            newPosX = newX - bl.x
            newPosY = newY - bl.y
